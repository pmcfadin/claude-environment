...
Use the following epic body template:
...
Use the following child issue body template:
...
- [ ] `[verification command]` [expected result]
- [ ] `npm test` (or project equivalent) passes with no failures
...
**Acceptance criteria must be mechanically verifiable.** Each criterion should ideally include a command or concrete check that a builder can self-verify and a reviewer can execute. Examples:
- Good: `` `grep -ri 'oldName' src/` returns zero results ``
- Good: `` `npm run build` exits with code 0 ``
- Good: `` `curl localhost:3000/api/health` returns `{"status":"ok"}` ``
- Bad: "Refactoring is clean and complete"
